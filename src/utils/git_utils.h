...
enum class branch_t : uint8_t { local, remote, all };
...
constexpr auto convert_to_delta_status_t(git_delta_t t) -> delta_status_t {
  ...
  case GIT_DELTA_UNMODIFIED:
    return delta_status_t::unmodified;
  case GIT_DELTA_ADDED:
    return delta_status_t::added;
  case GIT_DELTA_DELETED:
    return delta_status_t::deleted;
  case GIT_DELTA_MODIFIED:
    return delta_status_t::modified;
  case GIT_DELTA_RENAMED:
    return delta_status_t::renamed;
  case GIT_DELTA_COPIED:
    return delta_status_t::copied;
  case GIT_DELTA_IGNORED:
    return delta_status_t::ignored;
  case GIT_DELTA_UNTRACKED:
    return delta_status_t::untracked;
  case GIT_DELTA_TYPECHANGE:
    return delta_status_t::type_change;
  case GIT_DELTA_UNREADABLE:
    return delta_status_t::unreadable;
  case GIT_DELTA_CONFLICTED:
    return delta_status_t::conflicted;
  }
...
constexpr auto convert_to_file_mode_t(uint16_t m) -> file_mode_t {
  switch (m) {
  case 0000000:
    return file_mode_t::unreadable;
  case 0040000:
    return file_mode_t::tree;
  case 0100644:
    return file_mode_t::blob;
  case 0100755:
    return file_mode_t::blob_executable;
  case 0120000:
    return file_mode_t::link;
  case 0160000:
    return file_mode_t::commit;
  default:
...
constexpr auto convert_to_diff_line_t(char m) -> diff_line_t {
  switch (m) {
  case ' ':
    return diff_line_t::context;
  case '+':
    return diff_line_t::addition;
  case '-':
    return diff_line_t::deletion;
  case '=':
    return diff_line_t::context_eofnl;
  case '>':
    return diff_line_t::add_eofnl;
  case '<':
    return diff_line_t::del_eofnl;
  case 'F':
    return diff_line_t::file_hdr;
  case 'H':
    return diff_line_t::hunk_hdr;
  case 'B':
    return diff_line_t::binary;
  default:
...
constexpr auto convert_to_object_t(git_otype tp) -> object_t {
  ...
  case GIT_OBJ_TAG:
    return object_t::tag;
  case GIT_OBJ_BLOB:
    return object_t::blob;
  case GIT_OBJ_TREE:
    return object_t::tree;
  case GIT_OBJ_COMMIT:
    return object_t::commit;
  default:
    return object_t::any;
  }
  return object_t::any;
...
constexpr auto convert_to_repo_state(int state) -> repo_state_t {
  ...
  case GIT_REPOSITORY_STATE_NONE:
    return repo_state_t::none;
  case GIT_REPOSITORY_STATE_MERGE:
    return repo_state_t::merge;
  case GIT_REPOSITORY_STATE_REVERT:
    return repo_state_t::revert;
  case GIT_REPOSITORY_STATE_REVERT_SEQUENCE:
    return repo_state_t::revert_sequence;
  case GIT_REPOSITORY_STATE_CHERRYPICK:
    return repo_state_t::cherrypick;
  case GIT_REPOSITORY_STATE_CHERRYPICK_SEQUENCE:
    return repo_state_t::cherrypick_sequence;
  case GIT_REPOSITORY_STATE_BISECT:
    return repo_state_t::bisect;
  case GIT_REPOSITORY_STATE_REBASE:
    return repo_state_t::rebase;
  case GIT_REPOSITORY_STATE_REBASE_INTERACTIVE:
    return repo_state_t::rebase_interactive;
  case GIT_REPOSITORY_STATE_REBASE_MERGE:
    return repo_state_t::rebase_merge;
  case GIT_REPOSITORY_STATE_APPLY_MAILBOX:
    return repo_state_t::apply_mailbox;
  case GIT_REPOSITORY_STATE_APPLY_MAILBOX_OR_REBASE:
    return repo_state_t::apply_mailbox_or_rebase;
  default:
    unreachable();
  }
  unreachable();
...
/// TODO:
auto create_with_signature(repo_raw_ptr repo,
                           const std::string &commit_content) -> git_oid;
...
auto for_each(diff_raw_ptr diff, diff_file_cb file_cb, diff_binary_cb binary_cb,
              diff_hunk_cb hunk_cb, diff_line_cb line_cb, void *payload) -> int;
...
template <class T> auto peel(ref_raw_cptr ref) -> T {
  if constexpr (std::same_as<T, commit_ptr>) {
    ...
    auto ret =
        commit_ptr{reinterpret_cast<commit_raw_ptr>(raw), ::git_commit_free};
    return ret;
  }
...
template <typename T> auto convert(object_raw_ptr obj) -> T {
  auto type = object::type(obj);
  if constexpr (std::same_as<T, commit_raw_ptr> ||
                std::same_as<T, commit_raw_cptr>) {
    throw_if(type != object_t::commit,
             "The given object isn't commit_raw_ptr or commit_raw_cptr");
    return reinterpret_cast<T>(obj);
  }
  if constexpr (std::same_as<T, tree_raw_ptr> ||
                std::same_as<T, tree_raw_cptr>) {
    throw_if(type != object_t::tree,
             "The given object isn't tree_raw_ptr or tree_raw_cptr");
    return reinterpret_cast<T>(obj);
  }
  if constexpr (std::same_as<T, blob_raw_ptr> ||
                std::same_as<T, blob_raw_cptr>) {
    throw_if(type != object_t::blob,
             "The given object isn't blob_raw_ptr or blob_raw_cptr");
    return reinterpret_cast<T>(obj);
  }
  if constexpr (std::same_as<T, tag_raw_ptr> || std::same_as<T, tag_raw_cptr>) {
    throw_if(type != object_t::tag,
...
template <typename T> auto convert(object_ptr obj) -> T {
  auto *raw = obj.release();
  if constexpr (std::same_as<T, commit_ptr>) {
...
auto get_target_lines_in_hunk(git_patch &patch,
                              std::size_t hunk_idx) -> std::vector<std::string>;

auto get_source_lines_in_hunk(git_patch &patch,
                              std::size_t hunk_idx) -> std::vector<std::string>;
