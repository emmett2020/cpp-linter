...
template <noexcept_str_func Func> void throw_if(bool condition, Func &&func) {
  if (condition) {
...
inline auto concat(const std::vector<std::string> &strs,
                   char delim = '\n') -> std::string {
  return strs | std::views::join_with(delim) | std::ranges::to<std::string>();
}
