...
auto thread_safe_env_manager::get_instance() noexcept
    -> thread_safe_env_manager & {
...
void thread_safe_env_manager::set_cache(const std::string &name,
                                        const std::string &value) {
  auto lg = std::lock_guard(mutex_);
  cache_[name] = value;
}

void thread_safe_env_manager::set_cache(
