...
auto setup() -> int { return ::git_libgit2_init(); }
...
auto state(repo_raw_ptr repo) -> int { return ::git_repository_state(repo); }
...
auto for_each(diff_ptr diff, diff_file_cb file_cb, diff_binary_cb binary_cb,
...
auto deltas(diff_ptr diff) -> std::vector<diff_delta_detail> {
  auto line_cb = [](diff_delta_cptr cur_delta, diff_hunk_cptr cur_hunk,
                    diff_line_cptr cur_line, void *payload) -> int {
    ...
    auto delta_iter =
        std::ranges::find_if(deltas, [&](const diff_delta_detail &delta) {
          return delta.old_file.oid == cur_old_id &&
                 delta.new_file.oid == cur_new_id;
        });
    ...
    auto cur_hunk_header = std::string{
        static_cast<const char *>(cur_hunk->header), cur_hunk->header_len};
    auto hook_iter = std::ranges::find_if(
        delta_iter->hunks, [&](const diff_hunk_detail &hunk) {
          return hunk.header == cur_hunk_header;
        });
    if (hook_iter == delta_iter->hunks.end()) {
      auto hunk = diff_hunk_detail{.header = cur_hunk_header,
...
namespace object {
void free(object_ptr obj) { ::git_object_free(obj); }
...
namespace sig {
void free(signature_ptr sig) { git_signature_free(sig); }
