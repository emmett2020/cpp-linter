...
auto shutdown() -> int { return ::git_libgit2_shutdown(); }

auto delta_status_t_str(delta_status_t status) -> std::string {
  switch (status) {
  case delta_status_t::unmodified:
    return "unmodified";
  case delta_status_t::added:
    return "added";
  case delta_status_t::deleted:
    return "deleted";
  case delta_status_t::modified:
    return "modified";
  case delta_status_t::renamed:
    return "renamed";
  case delta_status_t::copied:
    return "copied";
  case delta_status_t::ignored:
    return "ignored";
  case delta_status_t::untracked:
    return "untracked";
  case delta_status_t::type_change:
    return "type_change";
  case delta_status_t::unreadable:
    return "unreadable";
  case delta_status_t::conflicted:
    return "conflicted";
  case delta_status_t::unknown:
    return "unknown";
  }
  return "unknown";
}
...
auto file_mode_t_str(file_mode_t mode) -> std::string {
  switch (mode) {
  case file_mode_t::unreadable:
    return "unreadable";
  case file_mode_t::tree:
    return "tree";
  case file_mode_t::blob:
    return "blob";
  case file_mode_t::blob_executable:
    return "blob_executable";
  case file_mode_t::link:
    return "link";
  case file_mode_t::commit:
    return "commit";
  case file_mode_t::unknown:
    return "unknown";
  }
  return "unknown";
}
...
auto diff_line_t_str(diff_line_t tp) -> std::string {
  switch (tp) {
  case diff_line_t::context:
    return "context";
  case diff_line_t::addition:
    return "addition";
  case diff_line_t::deletion:
    return "deletion";
  case diff_line_t::context_eofnl:
    return "context_eofnl";
  case diff_line_t::add_eofnl:
    return "add_eofnl";
  case diff_line_t::del_eofnl:
    return "del_eofnl";
  case diff_line_t::file_hdr:
    return "file_hdr";
  case diff_line_t::hunk_hdr:
    return "hunk_hdr";
  case diff_line_t::binary:
    return "binary";
  case diff_line_t::unknown:
    return "unknown";
  }
  return "unknown";
}
...
auto repo_state_str(repo_state_t state) -> std::string {
  switch (state) {
  case repo_state_t::none:
    return "none";
  case repo_state_t::merge:
    return "merge";
  case repo_state_t::revert:
    return "revert";
  case repo_state_t::revert_sequence:
    return "revert_sequence";
  case repo_state_t::cherrypick:
    return "cherrypick";
  case repo_state_t::cherrypick_sequence:
    return "cherrypick_sequence";
  case repo_state_t::bisect:
    return "bisect";
  case repo_state_t::rebase:
    return "rebase";
  case repo_state_t::rebase_interactive:
    return "rebase_interactive";
  case repo_state_t::rebase_merge:
    return "rebase_merge";
  case repo_state_t::apply_mailbox:
    return "apply_mailbox";
  case repo_state_t::apply_mailbox_or_rebase:
    return "apply_mailbox_or_rebase";
  }
  return "unknown";
}
...
auto is_same_file(const diff_file_detail &file1,
                  const diff_file_detail &file2) -> bool {
...
namespace config {
void free(config_raw_ptr config_ptr) { ::git_config_free(config_ptr); }

auto get_string(config_raw_cptr config_ptr,
...
void set_string(config_raw_ptr config_ptr, const std::string &key,
...
void set_bool(config_raw_ptr config_ptr, const std::string &key, bool value) {
  auto ret =
      ::git_config_set_bool(config_ptr, key.c_str(), static_cast<int>(value));
  throw_if(ret);
}

void set_int32(config_raw_ptr config_ptr, const std::string &key,
               int32_t value) {
  auto ret = ::git_config_set_int32(config_ptr, key.c_str(), value);
  throw_if(ret);
}

void set_int64(config_raw_ptr config_ptr, const std::string &key,
               int64_t value) {
...
auto create(repo_raw_ptr repo, const std::string &branch_name,
            commit_raw_cptr target, bool force) -> ref_ptr {
...
auto lookup(repo_raw_ptr repo, const std::string &name,
            branch_t branch_type) -> ref_ptr {
  auto convert_to = [&]() {
    switch (branch_type) {
    case branch_t::local:
      return git_branch_t::GIT_BRANCH_LOCAL;
    case branch_t::remote:
      return git_branch_t::GIT_BRANCH_REMOTE;
    case branch_t::all:
      return git_branch_t::GIT_BRANCH_ALL;
    }
...
auto current_name(repo_raw_ptr repo) -> std::string {
  ...
  throw_if(ref_type == ref_t::invalid,
...
namespace commit {
auto create(repo_raw_ptr repo, const std::string &updated_ref,
            signature_raw_cptr author, signature_raw_cptr committer,
            const std::string &message, tree_raw_cptr tree,
            std::vector<commit_raw_cptr> parents) -> git_oid {
  auto id = git_oid{};
  auto ret = ::git_commit_create(&id, repo, updated_ref.c_str(), author,
                                 committer, "UTF-8", message.c_str(), tree,
                                 parents.size(), parents.data());
  throw_if(ret);
  return id;
}

auto create_head(repo_raw_ptr repo, const std::string &message,
                 tree_raw_cptr index_tree) -> std::tuple<git_oid, commit_ptr> {
  auto sig = git::sig::create_default(repo);
  auto head_commit = repo::head_commit(repo);
  if (head_commit == nullptr) {
    auto oid =
        create(repo, "HEAD", sig.get(), sig.get(), message, index_tree, {});
...
namespace diff {
void free(diff_raw_ptr diff) { ::git_diff_free(diff); }

auto index_to_workdir(repo_raw_ptr repo, index_raw_ptr index,
...
auto tree_to_tree(repo_raw_ptr repo, tree_raw_ptr old_tree,
...
auto for_each(diff_raw_ptr diff, diff_file_cb file_cb, diff_binary_cb binary_cb,
              diff_hunk_cb hunk_cb, diff_line_cb line_cb,
              void *payload) -> int {
  return ::git_diff_foreach(diff, file_cb, binary_cb, hunk_cb, line_cb,
                            payload);
}
...
//     auto delta_iter = std::ranges::find_if(deltas, [&](const
//     diff_delta_detail &delta) {
//       return delta.old_file.oid == cur_old_id && delta.new_file.oid ==
//       cur_new_id;
//     });
//
...
//       std::string{static_cast<const char *>(cur_hunk->header),
//       cur_hunk->header_len};
//     auto hook_iter = std::ranges::find_if(delta_iter->hunks, [&](const
//     diff_hunk_detail &hunk) {
...
auto deltas(repo_raw_ptr repo, const std::string &spec1,
  ...
  throw_if(type1 == object_t::bad || type1 == object_t::blob,
           fmt::format("get deltas error since unknwon spec1: {}", spec1));
  throw_if(type2 == object_t::bad || type2 == object_t::blob,
           fmt::format("get deltas error since unknwon spec2: {}", spec2));
...
auto changed_files(repo_raw_ptr repo, const std::string &spec1,
                   const std::string &spec2) -> std::vector<std::string> {
...
auto changed_files(const std::unordered_map<std::string, git::diff_delta>
                       &deltas) -> std::vector<std::string> {
...
auto type(ref_raw_cptr ref) -> ref_t {
  ...
  case git_ref_t::GIT_REFERENCE_SYMBOLIC:
    return ref_t::symbolic;
  case git_ref_t::GIT_REFERENCE_DIRECT:
    return ref_t::direct;
  case git_ref_t::GIT_REFERENCE_ALL:
    return ref_t::all;
  }
  return ref_t::invalid;
}
...
void free(ref_raw_ptr ref) { ::git_reference_free(ref); }
...
namespace object {
void free(object_raw_ptr obj) { ::git_object_free(obj); }
...
auto id_str(object_raw_cptr obj) -> std::string { return oid::to_str(id(obj)); }
...
namespace sig {
void free(signature_raw_ptr sig) { git_signature_free(sig); }
...
namespace status {
auto gather(repo_raw_ptr repo,
            const status_options &options) -> status_list_ptr {
...
auto get_by_index(status_list_raw_ptr status_list,
                  std::size_t idx) -> status_entry_raw_cptr {
  const auto *ret = ::git_status_byindex(status_list, idx);
  throw_if(ret == nullptr,
           "get status list error since the given idx is out of range");
  return ret;
}
...
auto create_from_buffers(const std::string &old_buffer,
  ...
  auto ret = ::git_patch_from_buffers(
      &patch, old_buffer.data(), old_buffer.size(), old_as_path.data(),
      new_buffer.data(), new_buffer.size(), new_as_path.data(), &opts);
  throw_if(ret);
  return {patch, ::git_patch_free};
}
...
auto get_hunk(git_patch &patch,
              std::size_t hunk_idx) -> std::tuple<diff_hunk, std::size_t> {
...
auto get_hunk(patch_raw_ptr patch,
              std::size_t hunk_idx) -> std::tuple<diff_hunk, std::size_t> {
  const auto *hunk_ptr = diff_hunk_raw_ptr{nullptr};
  auto line_num = std::size_t{0};
  auto ret = ::git_patch_get_hunk(&hunk_ptr, &line_num, patch, hunk_idx);
...
auto num_lines_in_hunk(patch_raw_cptr patch,
                       std::size_t hunk_idx) -> std::size_t {
  return ::git_patch_num_lines_in_hunk(patch, hunk_idx);
}

auto get_line_in_hunk(patch_raw_ptr patch, std::size_t hunk_idx,
                      std::size_t line_idx) -> diff_line {
...
auto get_lines_in_hunk(patch_raw_ptr patch,
                       std::size_t hunk_idx) -> std::vector<std::string> {
  auto ret = std::vector<std::string>{};
...
auto get_lines_in_hunk(git_patch &patch,
                       std::size_t hunk_idx) -> std::vector<std::string> {
  auto ret = std::vector<std::string>{};
  size_t num_lines = patch::num_lines_in_hunk(&patch, hunk_idx);
  for (size_t i = 0; i < num_lines; i++) {
...
auto get_old_line_number(const git::diff_line &line)
...
auto get_new_line_number(const git::diff_line &line)
    -> std::optional<std::size_t> {
...
auto get_raw_content(repo_raw_ptr repo, tree_raw_cptr tree,
                     const std::string &file_name) -> std::string {
  throw_if(tree == nullptr,
           "failed to get raw content sicne tree is a null pointer");
  throw_if(file_name.empty(),
           "failed to get raw content sicne file name is empty");
...
auto get_raw_content(repo_raw_ptr repo, commit_raw_cptr commit,
                     const std::string &file_name) -> std::string {
