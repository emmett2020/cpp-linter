...
auto get_line_lens(std::string_view file_path) -> std::vector<uint32_t> {
  spdlog::trace("Enter clang_format::get_position() with file_path:{}",
                file_path);
...
auto parse_replacements_xml(std::string_view data) -> replacements_t {
  spdlog::trace("Enter clang_format::parse_replacements_xml() with data:{}",
                data);
...
auto make_replacements_options(std::string_view file)
  ...
  spdlog::trace("Enter clang_format::make_replacements_options() with file:{}",
                file);
...
auto make_source_code_options(std::string_view file)
    -> std::vector<std::string> {
  spdlog::trace("Enter clang_format::make_source_code_options() with file:{}",
                file);
  auto tool_opt = std::vector<std::string>{};
  tool_opt.emplace_back(file);
  return tool_opt;
...
auto execute(const user_option &user_opt, output_style_t output_style,
  ...
  spdlog::trace(
      "Enter clang_format::execute() with output_style:{}, repo:{}, file:{}",
      static_cast<int>(output_style), repo, file);
  ...
  spdlog::info("Running command: {} {}", user_opt.clang_format_binary,
               tool_opt_str);
...
void trace_shell_result(const shell::result &result) {
  spdlog::trace("The original result of clang-format:\nreturn code: "
                "{}\nstdout:\n{}stderr:\n{}",
                result.exit_code, result.std_out, result.std_err);
}

} // namespace

void trace_replacement(const replacement_type &replacement) {
  spdlog::trace("offset: {}, length: {}, data: {}", replacement.offset,
                replacement.length, replacement.data);
}
...
auto apply_on_single_file(const user_option &user_opt,
  ...
  spdlog::trace("Enter clang_format::apply_on_single_file() with "
                "needs_formatted_source_code:{}, "
                "repo:{}, file:{}",
                needs_formatted_source_code, repo, file);
  ...
  if (needs_formatted_source_code) {
    ...
    if (code_res.exit_code != 0) {
      return {.pass = false,
              .file = file,
              .replacements = {},
              .origin_stderr = code_res.std_err};
    }
