...
struct reporter_base {
  ...
  virtual auto
  make_issue_comment(const runtime_context &context) -> std::string = 0;

  virtual auto
  make_step_summary(const runtime_context &context) -> std::string = 0;

  virtual auto make_review_comment(const runtime_context &context)
      -> github::review_comments = 0;
