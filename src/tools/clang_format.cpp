...
auto get_pos(const std::vector<int32_t> &lens,
             int offset) -> std::tuple<int, int> {
  auto cur = 0;
...
auto parse_clang_tidy_header_line(std::string_view line)
...
auto execute(const option &option, std::string_view repo,
...
auto parse_stdout(std::string_view std_out) -> diagnostics {
  ...
  for (auto part : std::views::split(std_out, '\n')) {
    ...
    if (header_line) {
      spdlog::trace(" Result: {}:{}:{}: {}:{}{}", header_line->file_name,
                    header_line->row_idx, header_line->col_idx,
                    header_line->serverity, header_line->brief,
...
constexpr auto warning_and_error =
...
constexpr auto suppressed =
    R"(Suppressed (\d+) warnings \((\d+) in non-user code\)\.)";
...
auto parse_stderr(std::string_view std_err) -> statistic {
  ...
  auto suppressed_and_nolint_cb = [&](boost::smatch &match) {
    spdlog::trace(
        " Result: Suppressed {} warnings ({} in non-user code, {} NOLINT).",
...
void print_statistic(const statistic &stat) {
  ...
  spdlog::debug("Warnings treated as errors: {}",
                stat.warnings_treated_as_errors);
  spdlog::debug("Total suppressed warnings: {}",
                stat.total_suppressed_warnings);
...
auto run(const option &option, const std::string &repo,
  ...
  spdlog::info(
      "Successfully ran clang-tidy, now start to parse the output of it.");
  auto res = result{};
  res.pass = ec == 0;
  res.diags = parse_stdout(std_out);
  ...
  if (res.pass) {
    spdlog::info("The final result of ran clang-tidy on {} is: {}, detailed "
                 "information:\n{}",
                 file, "PASS", res.origin_stderr);
  } else {
    spdlog::error("The final result of ran clang-tidy on {} is: {} , detailed "
                  "information:\n{}",
                  file, "FAIL", res.origin_stderr);
  }
  return res;
}
