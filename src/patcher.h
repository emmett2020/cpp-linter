...
namespace linter {
  ...
  struct ReviewComments {
    ...
    std::unordered_map<std::string, int> tool_total{
      {  "clang-tidy", 0},
      {"clang-format", 0}
    };
    ...
    serialize_to_github_payload(std::string_view clang_tidy_version,
      ...
      for (const auto tool_name: tools) {
        auto tool_version = tool_name == "clang-tidy" ? clang_tidy_version : clang_format_version;

        // There is no tool version
        if (tool_version == "") {
          continue;
        }
        if (comments.size() > 0 && posted_tool_advice[tool_name] != tool_total[tool_name]) {
          summary += std::format(
            "Only {} out of {} {} concerns fit within this "
            "pull request's diff.\n",
            posted_tool_advice[tool_name],
            tool_total[tool_name],
            tool_name);
        }
