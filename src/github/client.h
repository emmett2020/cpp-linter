...
public:
  ...
  void get_issue_comment_id(const runtime_context &ctx) {
    spdlog::info("Start to get issue comment id for pull request: {}.",
                 ctx.pr_number);
    assert(ranges::contains(github_events_support_comments, ctx.event_name));

    auto path = fmt::format("/repos/{}/issues/{}/comments", ctx.repo_pair,
                            ctx.pr_number);
    auto headers =
        httplib::Headers{{"Accept", "application/vnd.github+json"},
    ...
    if (comments.is_null()) {
      spdlog::info("The pull request number {} doesn't have any comments yet",
                   ctx.pr_number);
      return;
    }
    ...
    if (comment == comments.end()) {
      spdlog::info(
          "The cpp-lint doesn't comments on pull request number {} yet",
          ctx.pr_number);
      return;
    }

    (*comment)["id"].get_to(comment_id_);
    spdlog::info("Successfully got comment id {} of pr {}", comment_id_,
                 ctx.pr_number);
  }
  ...
  void add_issue_comment(const runtime_context &ctx, const std::string &body) {
    ...
    const auto path = fmt::format("/repos/{}/issues/{}/comments", ctx.repo_pair,
                                  ctx.pr_number);
    const auto headers =
        httplib::Headers{{"Accept", "application/vnd.github.use_diff"},
                         {"Authorization", fmt::format("token {}", ctx.token)}};
    spdlog::info("Http request path: {}", path);
    ...
    spdlog::info("Successfully add new comment for pull-request {}, the new "
                 "added comment id is {}",
                 ctx.pr_number, comment_id_);
  }

  void update_issue_comment(const runtime_context &ctx,
    ...
    const auto path =
        fmt::format("/repos/{}/issues/comments/{}", ctx.repo_pair, comment_id_);
    const auto headers =
        httplib::Headers{{"Accept", "application/vnd.github.use_diff"},
                         {"Authorization", fmt::format("token {}", ctx.token)}};
    spdlog::info("Http request path: {}", path);
    ...
    spdlog::trace("Get github response body: {}", response->body);
    check_http_response(response);
    spdlog::info("Successfully updated comment {} of pr {}", comment_id_,
                 ctx.pr_number);
  }

  void add_or_update_issue_comment(const runtime_context &ctx,
                                   const std::string &body) {
    if (comment_id_ == -1) {
  ...
  void post_pull_request_review(const runtime_context &ctx,
                                const std::string &body) {
    spdlog::info("Start to post pull request review for pr number {}",
                 ctx.pr_number);

    const auto path =
        fmt::format("/repos/{}/pulls/{}/reviews", ctx.repo_pair, ctx.pr_number);
    const auto headers =
        httplib::Headers{{"Accept", "application/vnd.github.use_diff"},
                         {"Authorization", fmt::format("token {}", ctx.token)}};
    spdlog::info("Http request path: {}", path);
    ...
    spdlog::info("Successfully post pull_request_review for pull-request {}",
                 ctx.pr_number);
  }
  ...
  //   const auto path    = fmt::format("???", ctx_.repo, comment_id_);
  //   const auto headers = httplib::Headers{
  //     {"Accept", "application/vnd.github.use_diff"},
  //     {"Authorization", fmt::format("token {}", ctx_.token)}
  //   };
  ...
  //       auto dismiss_path = fmt::format("xxxx/{}/dismissals", id);
