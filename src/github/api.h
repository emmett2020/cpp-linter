...
public:
  explicit github_api_client(const runtime_context &ctx) : ctx_(ctx) {}
  ...
  static void check_http_response(const httplib::Result &response) {
    ...
    throw_unless(
        code == 1 || code == 2,
        std::format(
            "http response error since http status code: {}, reason: {}",
  ...
  void get_issue_comment_id() {
    spdlog::info("Start to get issue comment id for pull request: {}.",
                 ctx_.pr_number);
    assert(
        std::ranges::contains(github_events_support_comments, ctx_.event_name));

    auto path =
        std::format("/repos/{}/issues/{}/comments", ctx_.repo, ctx_.pr_number);
  ...
  void update_issue_comment(const std::string &body) {
    ...
    const auto path =
        std::format("/repos/{}/issues/comments/{}", ctx_.repo, comment_id_);
    const auto headers = httplib::Headers{
        {"Accept", "application/vnd.github.use_diff"},
        {"Authorization", std::format("token {}", ctx_.token)}};
    spdlog::info("Path: {}", path);
  ...
  void post_pull_request_review(const std::string &body) {
    ...
    const auto path =
        std::format("/repos/{}/pulls/{}/reviews", ctx_.repo, ctx_.pr_number);
    const auto headers = httplib::Headers{
        {"Accept", "application/vnd.github.use_diff"},
        {"Authorization", std::format("token {}", ctx_.token)}};
    spdlog::info("Path: {}", path);
...
private:
  const runtime_context &ctx_; // TODO: move context to parameter
