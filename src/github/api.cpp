...
void fill_context_by_env(const github_env &env, context &ctx) {
  ...
  throw_if(ctx.use_on_local, "The `fill_context_by_env` function must be only called on Github CI environment.");
  throw_unless(ctx.token.empty(), "The `token` will be automatically acquired by `GITHUB_TOKEN`. Don't set it from program option");
  throw_unless(ctx.repo.empty(), "The `repo` will be automatically acquired by `GITHUB_REPOSITORY` env. Don't set it from program option");
  throw_unless(ctx.repo_path.empty(), "The `repo_path` will be automatically acquired by `GITHUB_WORKSPACE` env. Don't set it from program option");
  throw_unless(ctx.event_name.empty(), "The `event_name` will be automatically acquired by `GITHUB_EVENT_NAME` env. Don't set it from program option");
  throw_unless(ctx.head_ref.empty(), "The `head_ref` will be automatically acquired by `GITHUB_REF` env. Don't set it from program option");
  throw_unless(ctx.head_commit.empty(), "The `head_commit` will be automatically acquired by `GITHUB_SHA` env. Don't set it from program option");
  ...
  if (std::ranges::contains(github_events_automatic_infer_base_ref, ctx.event_name)) {
    spdlog::debug("Github event is {}, so automatically use {} instead of {} as base ref",
                  env.event_name, ctx.base_ref, env.base_ref);
