...
void print_clang_tidy_total_result(const cpp_linter_result &result) {
  spdlog::info("Total changed file number: {}. While {} files are igored by "
               "user, {} files check "
               "is passed, {} files check is failed",
               result.patches.size(), result.clang_tidy_ignored_files.size(),
               result.clang_tidy_passed.size(),
               result.clang_tidy_failed.size());
}

auto make_clang_format_result_str(
    const context &ctx, const cpp_linter_result &result) -> std::string {
...
auto make_clang_tidy_result_str(
    const context &ctx, const cpp_linter_result &result) -> std::string {
  auto details = std::string{};
  details += std::format(
      "<details>\n<summary>{} reports:<strong>{} fails</strong></summary>\n",
      ctx.clang_tidy_option.clang_tidy_binary, result.clang_tidy_failed.size());
...
auto make_brief_result(const context &ctx,
                       const cpp_linter_result &result) -> std::string {
...
auto make_clang_format_pr_review_comment(
    const context &ctx, const cpp_linter_result &results,
  ...
  for (const auto &[file, result] : results.clang_format_failed) {
    ...
    auto format_source_patch = git::patch::create_from_buffers(
        old_buffer, file, result.formatted_source_code, file, opts);
...
auto make_clang_tidy_pr_review_comment([[maybe_unused]] const context &ctx,
...
void apply_clang_format_on_files(const context &ctx,
  ...
  for (const auto &file : changed_files) {
    ...
    auto result = clang_format::apply_on_single_file(
        opt, needs_formatted_source_code, ctx.repo_path, file);
    if (result.pass) {
    ...
    spdlog::error("file: {} doesn't pass {} check.", file,
...
void apply_clang_tidy_on_files(const context &ctx,
                               const std::vector<std::string> &changed_files,
                               cpp_linter_result &linter_result) {
