...
bool file_needs_to_be_checked(const std::string &iregex,
...
auto make_step_summary(const context &ctx,
...
auto make_pr_review_comment([[maybe_unused]] const context &ctx,
...
auto main(int argc, char **argv) -> int {
  ...
  if (ctx.clang_tidy_option.enable_clang_tidy) {
    ...
    for (const auto &file : changed_files) {
      ...
      if (result.pass) {
        ...
        spdlog::error("file: {} doesn't passes {} check.", file,
  ...
  if (ctx.clang_format_option.enable_clang_format) {
    ...
    for (const auto &file : changed_files) {
      ...
      if (result.pass) {
        spdlog::info("file: {} passes {} check.", file,
  ...
  if (ctx.enable_update_issue_comment) {
    ...
    github_client.add_or_update_issue_comment(std::format(
        "{} passed, failed: {}", linter_result.clang_tidy_passed.size(),
